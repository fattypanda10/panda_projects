        ...
        if attempt != 0:
            ...
            if check in guess_l:
                ...
                if letter in (string.ascii_lowercase and test_l) and letter not in used_stuff:
                    ...
                    # to prevent the guessed word from being printed on last try
                    if guess_l.count(check) != 0:
                        print('\n')
                        new_s = ''.join(guess_l)
                        print(new_s)
                        position = 0
                        used_stuff.append(letter)
                    elif guess_l.count(check) == 0:  # last attempt
                        new_s = ''.join(guess_l)
                        position = 0
                        used_stuff.append(letter)
...
while alpha != False:
    ...
    if option == "play":
        hangman()
        desire = input('Type "play" to play the game, "exit" to quit: ')
        if desire == "play":
            hangman()
        elif desire == "exit":
            alpha = False
        elif desire != "play" or desire != "exit":
            continue
